...
# ──────────────────────────────────────────────────
# Дешёвые фильтры по ключевым словам (без GPT)
# Чтобы простое сообщение ("привет") не гоняло 4 модели подряд.
# Если ключевых слов нет — пропускаем тяжёлый GPT-классификатор.
# ──────────────────────────────────────────────────
_FINANCE_KW = (
    "потрат", "купил", "куплю", "заплат", "оплат", "заработ", "получил", "доход",
    "расход", "трат", "долг", "кредит", "баланс", "отчёт", "отчет", "сум", "доллар",
    "млн", "тысяч", "занял", "одолжил", "вернул", "отдал", "погас", "деньги", "зарплат",
    "банк", "анорбанк", "узум", "avo", "аво", "pulinform", "юрлица", "рассрочк",
    "истам", "сирож", "навруз", "иван", "илёс", "илес", "умар", "лазиз", "$",
    "измени долг", "обнови долг", "обнови кредит", "исправь долг",
    "гугл таблиц", "google sheet", "обнови таблиц", "синхронизируй таблиц",
    "покажи долги", "покажи баланс", "финансовая ситуация",
)
_TRACKER_KW = (
    "спал", "выспал", "сон ", "сна", "вес ", "весе", " кг", "килограмм", "залип",
    "тикток", "tiktok", "инстаграм", "инста ", "соцсет", "дофамин", "привычк",
    "энергия", "бодрость", "стрик", "отметить",
)
_SMART_KW = (
    "задач", "сделать", "нужно", "создать", "создай", "запуст", "купить", "дедлайн",
    "напомни", "напомин", "remind", "через час", "через день", "через недел",
    "числа ", "что у меня", "покажи задач", "мои задачи", "список задач",
    "запомни", "что я знаю", "база знаний", "знани",
)


def _has_kw(text_lower: str, keywords) -> bool:
    return any(k in text_lower for k in keywords)


def _looks_financial(text_lower: str) -> bool:
    # ключевые слова ИЛИ число рядом с валютой/единицей
    if _has_kw(text_lower, _FINANCE_KW):
        return True
    return bool(re.search(r"\d+\s*(к|k|млн|тыс|сум|\$|руб)", text_lower))
...
def register(bot) -> None:
    ...
    def _process_buffered(user_id, bot_ref):
        ...
        try:
            ...
            # Финансовый ввод (только если есть финансовые ключевые слова)
            if _looks_financial(text_lower):
                from bot.services.finance import process_finance
                finance_result = process_finance(user_id, combined_text)
                if finance_result is not None:
                    _send_long(bot_ref, message, finance_result, parse_mode="Markdown")
                    return

            # Трекеры (только если есть ключевые слова трекеров)
            if _has_kw(text_lower, _TRACKER_KW):
                from bot.services.trackers import process_tracker
                tracker_result = process_tracker(user_id, combined_text)
                if tracker_result is not None:
                    _send_long(bot_ref, message, tracker_result, parse_mode="Markdown")
                    return

            # Умный ввод (задачи/напоминания/запросы — только при ключевых словах)
            if _has_kw(text_lower, _SMART_KW):
                ...
                if smart_result is not None:
                    _send_long(bot_ref, message, smart_result)
                    return
