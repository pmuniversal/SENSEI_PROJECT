...
class SheetsManager:
    ...
    def _init_credits_sheet(self):
        ...
        ВАЖНО: данные засеваются ТОЛЬКО если лист пустой. Если в таблице уже
        есть записи — НЕ перезаписываем, чтобы сохранить обновлённые остатки
        (иначе погашения долгов сбрасывались бы при каждом перезапуске бота).
        """
        if not self.service or not self.sheet_id:
            return

        # Проверяем — есть ли уже данные (строка 2, первая колонка)
        try:
            existing = self.service.spreadsheets().values().get(
                spreadsheetId=self.sheet_id,
                range=f"{SHEET_CREDITS}!A2:A2"
            ).execute()
            if existing.get("values"):
                print(f"[SHEETS] Лист '{SHEET_CREDITS}' уже заполнен — пропускаю засев (сохраняю остатки)")
                return
        except Exception:
            pass  # если не удалось прочитать — продолжаем засев
        ...
        try:
            ...
            credits = [
                ...
                ["Pulinform (рассрочка)", "25,000,000", "25,000,000", "0%", "2,800,000",
