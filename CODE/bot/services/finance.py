...
- category: категория расхода/дохода. Используй ТОЛЬКО эти значения:
  Расходы: питание, транспорт, вернул_долг, отдал_в_долг, бизнес/инвестиции,
           здоровье, покупки, дом.хозяйство, саморазвитие, развлечения, семья,
           аренда, кредит, зарплата_сотруднику, другое, неизвестно
  Доходы: зарплата, фриланс, youtube, услуги, инвестиции, подарок, другое
- payment_method: "наличные" | "карта" | "перевод" | null
...
def process_finance(user_id, text: str) -> str | None:
    ...
    # Неизвестный тип или слишком сложное сообщение — пусть AI обработает
    return None
